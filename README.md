...
```sh
omp plugin install 'git+https://github.com/ericjuta/omp-cljs-codemode.git#v0.1.5'
```
...
After success, run `omp plugin list`. It must show `@ericjuta/omp-cljs-codemode@0.1.5`. If the historical identity is still listed, remove it only now; skip these cleanup commands when it is already absent:
...
bun run eval:guidance --model provider/model \
  ...
  --output evals/results/candidate-v0.1.5.json
