...
The repository is public. Install the immutable public v0.1.19 release directly over HTTPS; no GitHub credentials are required:
...
After success, run `omp plugin list`. It must show `@ericjuta/omp-cljs-codemode@0.1.19`. If the historical identity is still listed, remove it only now; skip these cleanup commands when it is already absent:
...
bun run eval:guidance --model provider/model \
  ...
  --output evals/results/candidate-v0.1.19.json
